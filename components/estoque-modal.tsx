...
export function EstoqueModal({ isOpen, onClose, product }: EstoqueModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-card rounded-lg shadow-xl w-full max-w-4xl mx-4 p-6 space-y-6 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-foreground">Estoque do Produto</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
          <div>
            ...
            <p className="text-sm font-medium text-foreground">{product.MARCA || '-'}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Unidade</p>
            <p className="text-sm font-medium text-foreground">{product.UNIDADE || '-'}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Ativo</p>
            <p className="text-sm font-medium text-foreground">{product.ATIVO === 'S' ? 'Sim' : 'Não'}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Estoque Total</p>
            <p className="text-sm font-bold text-primary">{formatQuantity(estoqueTotal.toString())}</p>
          </div>
        </div>
          ...
          <div className="max-h-[400px] overflow-y-auto">
            <table className="w-full">
              <thead className="sticky top-0 z-10 bg-secondary">
                <tr>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-sidebar-foreground uppercase tracking-wider">
                    Local
                  </th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-sidebar-foreground uppercase tracking-wider">
                    Controle
                  ...
                  <th className="px-6 py-3 text-right text-sm font-semibold text-sidebar-foreground uppercase tracking-wider">
                    Quantidade
                  </th>
                </tr>
              </thead>
                ...
                {estoques.length === 0 ? (
                  ...
                  estoques.map((estoque) => (
                    <tr key={estoque._id} className="hover:bg-muted/50 transition-colors">
                      <td className="px-6 py-3 text-sm text-foreground">{estoque.CODLOCAL}</td>
                      <td className="px-6 py-3 text-sm text-foreground">{estoque.CONTROLE || '-'}</td>
              ...
              {estoques.length > 0 && (
                <tfoot className="bg-secondary">
                  <tr>
                    <td colSpan={3} className="px-6 py-3 text-sm font-semibold text-sidebar-foreground text-right">
                      TOTAL:
                    </td>
